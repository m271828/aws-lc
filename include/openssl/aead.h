...
/**
 * @name AEAD Algorithms
 *
 * @{
 */

/**
 * @brief AES-128 in Galois Counter Mode.
 *
 * @details
...
/**
 * @brief AES-192 in Galois Counter Mode.
 *
 * @details
 * Note: AES-GCM should only be used with 12-byte (96-bit) nonces. Although it
 * is specified to take a variable-length nonce, nonces with other lengths are
...
/**
 * @brief AES-256 in Galois Counter Mode.
 *
 * @details
 * Note: AES-GCM should only be used with 12-byte (96-bit) nonces. Although it
 * is specified to take a variable-length nonce, nonces with other lengths are
...
/**
 * @brief AEAD built from ChaCha20 and Poly1305 as described in RFC 8439.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_chacha20_poly1305(void);

/**
 * @brief ChaCha20-Poly1305 with an extended nonce.
 *
 * @details
...
/**
 * @brief AES-128 in CTR mode with HMAC-SHA256 for authentication.
 *
 * @details
 * The nonce is 12 bytes; the bottom 32-bits are used as the block counter, thus
 * the maximum plaintext size is 64GB.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_aes_128_ctr_hmac_sha256(void);

/**
 * @brief AES-256 in CTR mode with HMAC-SHA256 for authentication.
 *
 * @details
 * See #EVP_aead_aes_128_ctr_hmac_sha256 for details.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_aes_256_ctr_hmac_sha256(void);

/**
 * @brief AES-128 in GCM-SIV mode.
 *
 * @details
 * See RFC 8452.
 */
...
/**
 * @brief AES-128 in Galois Counter Mode with internal nonce generation.
 *
 * @details
 * The 12-byte nonce is appended to the tag and is generated internally. The
 * "tag", for the purposes of the API, is thus 12 bytes larger. The nonce
 * parameter when using this AEAD must be 0-length. Since the nonce is random, a
 * single key should not be used for more than 2^32 seal operations.
 *
...
/**
 * @brief AES-256 in Galois Counter Mode with internal nonce generation.
 *
 * @details
 * The 12-byte nonce is appended to the tag and is generated internally. The
 * "tag", for the purposes of the API, is thus 12 bytes larger. The nonce
...
/**
 * @brief AES-128-CCM with M=4 and L=2.
 *
 * @details
 * Uses 4-byte tags and 13-byte nonces as described in the Bluetooth Core
 * Specification v5.0, Volume 6, Part E, Section 1.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_aes_128_ccm_bluetooth(void);

/**
 * @brief AES-128-CCM with M=8 and L=2.
 *
 * @details
 * Uses 8-byte tags and 13-byte nonces as used in the Bluetooth Mesh Networking
 * Specification v1.0.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_aes_128_ccm_bluetooth_8(void);

/**
 * @brief AES-128-CCM with M=16 and L=2.
 *
 * @details
 * Uses 16-byte tags and 13-byte nonces as used in the Matter specification.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_aes_128_ccm_matter(void);

/**
 * @brief Tests for hardware support for AES-GCM.
 *
 * @details
 * Returns 1 if we enable hardware support for fast and constant-time AES-GCM.
 */
...
/**
 * @brief Returns the length of the keys in bytes used by `aead`.
 */
OPENSSL_EXPORT size_t EVP_AEAD_key_length(const EVP_AEAD *aead);

/**
 * @brief Returns the length of the per-message nonce in bytes for `aead`.
 *
 * @details
 * Some `aead`'s might support a larger set of nonce-lengths (e.g. aes-gcm).
 */
OPENSSL_EXPORT size_t EVP_AEAD_nonce_length(const EVP_AEAD *aead);

/**
 * @brief Returns the maximum number of additional bytes added when sealing data with `aead`.
 */
OPENSSL_EXPORT size_t EVP_AEAD_max_overhead(const EVP_AEAD *aead);

/**
 * @brief Returns the maximum tag length when using `aead`.
 *
 * @details
 * This is the largest value that can be passed as `tag_len` to
 * #EVP_AEAD_CTX_init.
 */
 ...
 *
 * @brief An evp_aead_ctx_st represents an AEAD algorithm and configuration.
 *
 * @details
 * Configuration includes a specific key and message-independent IV.
 */
struct evp_aead_ctx_st {
  const EVP_AEAD *aead;
  union evp_aead_ctx_st_state state;
  uint8_t state_offset;
  /** May contain the actual length of the authentication tag if it is known at initialization time. */
  uint8_t tag_len;
};

/**
 * @def EVP_AEAD_MAX_KEY_LENGTH
 *
 * @brief Contains the maximum key length used by any AEAD defined in this header.
 */
#define EVP_AEAD_MAX_KEY_LENGTH 80

/**
 * @def EVP_AEAD_MAX_NONCE_LENGTH
 *
 * @brief Contains the maximum nonce length used by any AEAD defined in this header.
 */
#define EVP_AEAD_MAX_NONCE_LENGTH 24

/**
 * @def EVP_AEAD_MAX_OVERHEAD
 *
 * @brief Contains the maximum overhead used by any AEAD defined in this header.
 */
#define EVP_AEAD_MAX_OVERHEAD 64

/**
 * @def EVP_AEAD_DEFAULT_TAG_LENGTH
 *
...
/**
 * @brief Sets an uninitialized `ctx` to the 0 state.
 *
 * @details
 * It must be initialized with #EVP_AEAD_CTX_init before use. It is safe, but
 * not necessary, to call #EVP_AEAD_CTX_cleanup in this state. This may be used
 * for more uniform cleanup of EVP_AEAD_CTX.
 */
OPENSSL_EXPORT void EVP_AEAD_CTX_zero(EVP_AEAD_CTX *ctx);

/**
 * @brief Allocates an EVP_AEAD_CTX, calls #EVP_AEAD_CTX_init.
 *
 * @returns Allocated EVP_AEAD_CTX or `NULL` on error.
 */
...
/**
 * @brief Calls #EVP_AEAD_CTX_cleanup and #OPENSSL_free on `ctx`.
 */
OPENSSL_EXPORT void EVP_AEAD_CTX_free(EVP_AEAD_CTX *ctx);

/**
 * @brief Initializes `ctx` for the given AEAD algorithm.
 *
 * @details
 * The `impl` argument is ignored and should be `NULL`. Authentication tags may
 * be truncated by passing a size as `tag_len`. A `tag_len` of 0 indicates the
 * default tag length and this is defined as #EVP_AEAD_DEFAULT_TAG_LENGTH for
 * readability.
 *
 * If an error is returned, you do not need to call #EVP_AEAD_CTX_cleanup, but
 * it's harmless to do so.
 *
 * @retval 1 Success
 * @retval 0 Failure (pushes to the error stack).
 */
...
/**
 * @brief Frees any data allocated by `ctx`.
 *
 * @details
 * It is a no-op to call on an EVP_AEAD_CTX that has been `memset` to all 0's.
 */
OPENSSL_EXPORT void EVP_AEAD_CTX_cleanup(EVP_AEAD_CTX *ctx);

/**
 * @brief Encrypts and authenticates bytes from `in` to `out`.
 *
 * @details
 * Encrypts and authenticates `in_len` bytes from `in` and authenticates
 * `ad_len` bytes from `ad` and writes the result to `out`. It returns 1 on
 ...
 * insufficient, 0 will be returned. If any error occurs, `out` will be
 * filled with 0 bytes and `*out_len` set to 0.
 *
 * If `in` and `out` alias then `out` must equal `in`.
 */
...
/**
 * @brief Authenticates bytes from `in` and `ad` and decrypts into `out`.
 *
 * @details
 * Authenticates `in_len` bytes from `in` and `ad_len` bytes from `ad` and
 * decrypts at most `in_len` bytes into `out`. It returns 1 on success and 0
 * otherwise.
 *
 * This function may be called concurrently with itself or any other seal/open
 * function on the same EVP_AEAD_CTX.
...
/**
 * @brief Encrypts and authenticates bytes from `in` and `ad`.
 *
 * @details
 * Encrypts and authenticates `in_len` bytes from `in` and authenticates
 * `ad_len` bytes from `ad`. It writes `in_len` bytes of ciphertext to `out`
 * and the authentication tag to `out_tag`. It returns 1 on success and 0
 * otherwise.
 *
 * This function may be called concurrently with itself or any other seal/open
 * function on the same EVP_AEAD_CTX.
 *
 * Exactly `in_len` bytes are written to `out`, as well as up to
 * #EVP_AEAD_max_overhead and `extra_in_len` bytes to `out_tag`. On successful
 ...
 * `max_out_tag_len` is insufficient, 0 will be returned. If any error occurs,
 * `out` and `out_tag` will be filled with 0 bytes and *out_tag_len set to 0.
 *
 * If `in` and `out` alias then `out` must be equal `in`. `out_tag` may not alias
 * any other argument.
 */
...
/**
 * @brief Decrypts and authenticates from `in` and authenticates `ad` using `in_tag`.
 *
 * @details
 * Decrypts and authenticates `in_len` bytes from `in` and authenticates
 * `ad_len` bytes from `ad` using `in_tag_len` bytes of authentication tag from
 * `in_tag`. If successful, it writes `in_len` bytes of plaintext to `out`. It
 ...
 * occurs, `out` will be filled with 0 bytes.
 *
 * If `in` and `out` alias then `out` must equal `in`.
 */
...
/**
 * @brief Returns the underlying AEAD for `ctx` or `NULL` if one has not been set.
 */
...
/**
 * @name TLS-specific AEAD Algorithms
 *
 * @details
...
/**
 * @brief AES-128 in Galois Counter Mode using the TLS 1.2 nonce construction.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_aes_128_gcm_tls12(void);

/**
 * @brief AES-256 in Galois Counter Mode using the TLS 1.2 nonce construction.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_aes_256_gcm_tls12(void);

/**
 * @brief AES-128 in Galois Counter Mode using the TLS 1.3 nonce construction.
 */
OPENSSL_EXPORT const EVP_AEAD *EVP_aead_aes_128_gcm_tls13(void);

/**
 * @brief AES-256 in Galois Counter Mode using the TLS 1.3 nonce construction.
 */
 ...
 *
 * @brief Denotes the direction of an AEAD operation.
 */
enum evp_aead_direction_t {
  evp_aead_open,
  evp_aead_seal
...
/**
 * @brief Calls #EVP_AEAD_CTX_init for normal AEADs.
 *
 * @details
 * For TLS-specific and SSL3-specific AEADs, it initializes `ctx` for a given
 * direction.
 */
...
/**
 * @brief Sets the length of the IV and allocates the buffer.
 *
 * @details
 * Sets `*out_len` to the length of the IV for `ctx` and sets `*out_iv` to
 * point to that many bytes of the current IV. This is only meaningful for
 * AEADs with implicit IVs (i.e. CBC mode in TLS 1.0).
 *
 * @retval 1 Success
 * @retval 0 Error
 */
...
/**
 * @brief Computes the exact byte length of tag.
 *
 * @details
 * Computes the exact byte length of the tag written by
 * #EVP_AEAD_CTX_seal_scatter and writes it to `*out_tag_len`. It returns 1
 * on success or 0 on error. `in_len` and `extra_in_len` must equal the
...
/**
 * @def FIPS_AES_GCM_NONCE_LENGTH
 */
#define FIPS_AES_GCM_NONCE_LENGTH 12

/**
 * @brief Computes a deterministic IV from an IPv4 address and time since boot.
 *
 * @details
 * Computes a deterministic IV compliant with NIST SP 800-38D, built from an
 * IPv4 address and the number of nanoseconds since boot, writing it to `out_iv`
 * (in little endian). It returns 1 on success or 0 for error.
 *
